...
const APIKEY = "VOJP9jAPOAnpFoqrglPpEC4BzwNoU6vJ";
  ...
  const locateMe = () => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => getCurrentLocation(position),
        errLoc,
        {
  ...
  return (
    <div className="inputBox">
      <input
        className="inp"
        value={start}
        onChange={(e) => getSearchResults(e, setStart, start)}
      ...
      {placeholder === "Pickup point" ? (
        <div className="autoLocation">
          ...
          <label className="label" for="check" onClick={() => locateMe()}>
            Auto Locate
          </label>
        </div>
      ) : null}
